...
- **product-reviews** — The primary dataset you build from scratch. 10 examples are generated by running the structured-output agent on review texts, and 3 are added manually. No train/test splits are assigned.
- **product-reviews-from-traces** — A secondary dataset created by querying your existing LangSmith traces and converting their inputs/outputs into examples. The example count depends on how many traces were captured with both input and output data.
- **product-reviews-csv** — A dataset imported from a CSV file. Contains 3 examples organized into `train` and `test` splits to demonstrate how split metadata works for evaluation.
