...
**1. (Concept)** The lab builds three separate memory mechanisms. Name all three, say which one survives a new `thread_id`, and what happens to the *conversation* of evening 1 when Amara returns in evening 2. *(See Section 7 and Steps 7–8.)*

**2. (Code)** Decode this call: `store.put(("guests", "amara", "facts"), "fact-2", {"content": "Amara loves tiramisu."})`. What are the namespace, the key, and the value? Write the one-liner that reads back **every** fact stored for Amara, and the one-liner that would read Bob's instead. *(See Section 7 and Step 3.)*

**3. (Concept)** Why can't Bob's evening ever show Amara's facts, even though both guests share one `InMemoryStore`? What is the mechanism, and how does the same idea keep multi-tenant SaaS memory safe? *(See Section 7 and Step 9.)*
...
**2.** Namespace: `("guests", "amara", "facts")`. Key: `"fact-2"`. Value: `{"content": "Amara loves tiramisu."}`. Read Amara's facts: `store.search(("guests", "amara", "facts"))`; Bob's: `store.search(("guests", "bob", "facts"))`. `search` is prefix-based, so the first tuple element tells the store *which guest's* branch to walk.

**3.** Namespacing. The store is a tree keyed by tuple prefixes; Amara lives under `("guests", "amara", ...)` and Bob under `("guests", "bob", ...)`. `search(("guests", "amara", "facts"))` only walks Amara's branch, so Bob's context never receives Amara's items. In multi-tenant systems the tenant id becomes the namespace root — `("tenant_42", "users", ...)` — and every read is naturally scoped to one tenant with no shared-state bugs.
